...
| L4 | Runner robustness | A run can never leave the sim + interface running; status line truthful on a 1 GB trace; 64-bit host enforced; no inherited stdout pipe. | Executor RUNNING in the main checkout (scripts only). | Review -> commit -> REQUIRED before G7. | STANDARD | none |
...
| L6 | User-owned | Licence renewal (row 12, lapses 2026-09-15); DI-Guy data package (row 17, any lifeform run); TASKABRT code; C:\MAK sanctions above. | Open. | - | RULE | yes |
