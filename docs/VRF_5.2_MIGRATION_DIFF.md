...
| # | 5.0.2 | 5.2d | Cite | Effect | Decision |
|---|---|---|---|---|---|
| D1 | Ground entity move-to-a-point = 4 tasks (move-to-location, -path-plan, move-to-waypoint, -path-plan); road use chosen by task id | One ground-vehicle-move-to with a planning phase, automatic road use, dynamic obstacle avoidance ON, Gameware/collision sensors gone; old ids auto-convert on scenario load, but "remote-control applications that issue movement tasks to ground vehicles may need to be updated" | MG 2.4 p18-20 Table 1; API52 Ground Vehicle Movement; RN p7 | **RULED, NO PROBE OWED - see DECISION_EVIDENCE Y-10, which SUPERSEDES this cell.** `moveAlongRoute` is byte-identical 5.0.2/5.2d (vrfRemoteController.h:1638), undeprecated. `DtMoveToLocationTask` is DELETED (moveToLocationTasks.h = a 15-line `#error` shim) so the probe has no class to probe; moveToTask.h:181-184 "move-to is deprecated for ground vehicles". VENDOR CONFIRMS THE MAPPING (help ConceptsEntityLevel/GroundVehMove/EntityMovementGroundVehicleOverview.htm): "Move To ... plan a route on roads and around obstacles ... then following that planned route"; "Move Along Route ... does not use road movement, nor does the entity plan paths before moving". An AUTHORED C2SIM route must therefore use MoveAlongRoute - Move To would hand route choice to the planner. KEEP MoveAlongRoute; the MoveToLocation/PlanAndMoveTo probe is RETIRED. New 5.2 behaviour to expect in traces (help vrf_groundMovementExecution.htm): a blocked vehicle backs up and, after repeated failures, makes ONE replan from its current position | Y-10 RULED |
