...
(STP-820/823/822). RESOLVED BY THE V6f HARVEST (16:02Z run, V6_LIVE_JOIN_GATE sec 12, e67e084): the R5 order's waypoints are in SWEDEN (golden STP init) while the R9 lean init's units are in the MOJAVE - our own 'Terrain profile' line showed it; the interface authored an 8,769 km leg and the back end's off-road path job never returns (ws 0.8-2.2 GB/min, status lost ~112 s after dispatch; the platoon-only order fails alike, company fan-out FALSIFIED). Tools were fine (STP-820). STP-822 liveness LIVE PASS (1 loss report, 1 TASKABRT, 0 stale position reports); WS tripwire fired 6x (alerts file only - nothing acts on it yet). OWED: V6g = the same three tasks with MOJAVE coordinates (PROBE_V6G_MOJAVE_Order.xml, PREREG_V6C amendment 6) - HIT closes STP-823 as a DATA defect and drives STP-833 (refuse out-of-terrain routes); MISS promotes the vendor flat-query SMS. STP-825 (rtiexec rejects creates ~1 in 4-7; wire capture: sender byte-perfect -> vendor receive path; join-path holder = runner Stage 2h) and STP-832 (bridge no longer crashes on a refused create, merged 13e8c73) are the launch-side state.
