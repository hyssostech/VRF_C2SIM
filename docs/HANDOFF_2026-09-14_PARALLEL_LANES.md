...
(STP-820/823/822). The R5 ORDER sends the back end into a MEMORY RUNAWAY (1-2.5 GB/min, low CPU; 3 -> 31 GB; thread samples in runs/launch52) on BOTH fixtures; status/frames/units stop 121 s after dispatch; the ridge order stays flat at 4 GB. MERGED 14:52Z main 3c71025: STP-822 liveness (default ON, offline 25/0 + fail-first 11) + the ws runaway tripwire. V6f STILL OWED: three launches died at the sim's federation CREATE (STP-825: rtiexec 5.0.1 FOM Reader 'Extra content at the end of the document', sim 3/3 vs RtiProbe 6/6, cause open; JOIN-path HOLDER workaround CONFIRMED 15:20Z, RUNBOOK 9c; rtiexec restart = user's call) and one at Stage 8 (STP-830: the SDK's DetermineProtocol took '<Task>' inside the order file's comment as the root -> BML -> plain-text reply -> PushOrder crash; data file fixed, tool hardening merged next). Next launch = holder + v6f_go.sh, unchanged recipe.
