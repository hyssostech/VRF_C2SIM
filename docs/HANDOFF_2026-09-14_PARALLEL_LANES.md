...
4. DEMO-READY residue (readiness rows 9, 13, 15, 18), then the STP TASK VOCABULARY (L8) - the user's
   stated goal beyond MOVE - and then the aggregate-level profile (Y-15). D1 STOPPED on teardown (vrfGui exit prompt, STP-844/845, fixed); D1b/D2 PASS (0.60x speed open); route shift default ON merged f26d4ad; D3 PASS; STP-825: RID levers refuted; notify-level pilot + 16-instance replication both without a registered verdict (direction 0/126 vs 19/126, not proven); mechanism less settled (79% SILENT refusals); demo posture settled = persistent holder + -n 3; MAK case is the user's call; A+C merged adca180; D6/D7 runner truth 7/7, C confirmed (disjoint footprints, jam PASS), A = 0.0 s on R9, N4 hex-ring centroid shift, N7 sim not real time; D8 on 1d0fb69: N4 FIXED (published position 0.0 m, geometric correctness, no speed gain, +28 s vs D7); P1/P2/P3/P5 HIT, P4 registered-quantity HIT but its MISS-clause wording is defective (sim/wall is load-dependent, 2.6-4.7x, not a constant 3x); join-detector fix (9711f46) proven live 6/6 vs 0/6 against a garbled rtiexec sink; D5/D5b Way B rehearsal: steps 1-3 HIT live for the first time (LaunchVrf52's own holder + Say-Info dead-branch fixed 1553e48), then the back end crashed 1.3 s after the order (STP-854, n=1, undiagnosed) with a dead-sim-reported-healthy finding alongside it (STP-853) and no order-timing gate (STP-852, fix lane in progress) - rows 5/7/13 reopened, C2 owed; Iron Storm cut A blocked at the nav gate (only T14 passes) - decision with the user.
