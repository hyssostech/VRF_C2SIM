...
6. **Which makData 19 terrain parts (2-4) does the target AO need?** Only part 1 and TestTerrain are loaded (sec 1). VERIFIED 2026-09-02: part 1 is NOT enough - `TerrainManifest1Of4.txt` lists Hawaii/VR-Village/Default_Stage_12/LittlePond/Texas/Kabul and 2 files under `Terrain/World`; SharedData9 has NO `MAK Earth Space (online).mtf` (the geocentric terrain every fixture uses) and NO `Terrain\World\Elevation`, both present in SharedData. Install parts 2-4 (SharePoint Home/Software/MAK/5.2d/Data19, 1.67+1.34+1.60 GB) before any 5.2 fixture run; the part manifests will then show which one carried it.
