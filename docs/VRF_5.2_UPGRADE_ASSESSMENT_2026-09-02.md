...
Both trees coexist, and nothing under `vrforces5.2d\appData\settings` names a SharedData version (VERIFIED: no match in that tree), so version selection is by install-time data path, not a settings file we must edit. UPDATE 2026-09-02 (evening): all four terrain parts are installed (`TerrainDataVersionInfo_1..4.txt`; `Terrain\World\{Cgf,Elevation,Features,Geometry,Imagery}` present). **VERIFIED, BLOCKING for the fixtures:** makData 19 has NO `MAK Earth Space (online)` terrain - absent from all four `TerrainManifest*Of4.txt`, present in 16's `TerrainsDataManifest.txt`. Every fixture (`C2simEx`, R9, COA-STP1, TropicTortoise base) loads `MAK Earth Space (online).mtf`. `MAK Earth Air and Space (online)` (new in 19) is NOT a rename: its .earth is an air-management terrain with elevation capped at LOD 10 (~90 m). Candidate successor for ground work: `MAK Earth (online).mtf` (in both 16 and 19). Side finding: 16's `MAK Earth Space (online).mtf` is dated 2026-07-14 11:59 and references `navData\MAK Earth Space (online)\NavArea-ground-platform 1.navRuntimeConfig` = the DISABLED NavArea artifact; the 5.2 re-point leaves it behind by construction. Step 4 gains this re-point; open question 6 restated.
...
| # | step | gate | why |
|---|---|---|---|
| 1 | DONE 2026-09-02: docs installed by MAK's doc installer into `vrforces5.2d\doc` (incl. the 5.2 Model Catalog); public copies also in `docs/vendor/mak-5.2/`; PDBs installed | - | sec 0: re-verify this document's prose claims against the real Migration Guide + Users Guide before step 4 |
...
| 4 | Port `C2simEx` to 5.2: drop superType from the 4 templates, re-point the include, re-check the eight `.mtl` payloads against 5.2 defaults, AND re-point the terrain from `MAK Earth Space (online).mtf` (gone in makData 19) to its successor - `MAK Earth (online).mtf` proposed; elevation source changes, so R9's terrain-frame / TerrainProfile results must be re-verified (PREREG) | **PLAN** | sec 3.1, 4.1 |
...
6. ~~Which makData 19 terrain parts does the AO need~~ DONE 2026-09-02: all four installed. REPLACED BY: **which 19 terrain replaces `MAK Earth Space (online)` for the fixtures?** (sec 1 evening update; `MAK Earth (online).mtf` proposed, user ruling needed - it changes the elevation source under every R9 mechanism result.)
