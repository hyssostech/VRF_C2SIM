...
### D9 RESULT (run 20260921T094051Z)

P1 MISS AS REGISTERED on the quoted string only: the app printed "(5 empty shell(s))", not the registered "(4 ...)" - 5 IS RIGHT and the prereg's 4 is wrong (it copied the build report, which copied a pre-existing mislabelled C13 log line, N15); HIT on every other limb - the barrier line fires before "Init dispatched", READY TO TASK fires once 0.9 s before the order (measured from the barrier's own arming, the tightest bound the app log allows), every forbidden string (WAITING FOR THE BACK END, ORDER BEFORE READY TO TASK, MATERIALIZE...HELD, REFUSING TO MATERIALIZE, READY TO TASK - NOT REACHED) is zero, and the MATERIALIZE sequence at order receipt is D8's shape line for line.
P2 MISS on two limbs. (a) N13, a REAL DEFECT, filed as STP-855: T_R5_CO1's route origin sits 50.4 m at bearing 119.9 deg off the authored coordinate (route 1,139 m against the registered 1,110-1,116 m band, bar 569 m, radius 285 m) because the app reads the composed parent's published position inside VR-Forces' own re-compose transient after the platoon children are delete/re-created; D8's "N4 FIXED, 0.0 m" was won by about 0.5 s of timing luck, not by the geometry being fixed, and that claim is WITHDRAWN as a property of build 4f1f149 (D5c independently reproduces this - it also won the race, by 0.39 s, and reads the same R/4-off-ring signature before decaying); fix lane fix/composed-parent-route-origin in progress, recommended fix = build the origin from the children's own positions rather than from the parent's published one. (b) T_R5_TK1 closed at +27.4% SIM vs D8's figure - one 5 s arrival-check interval on the shortest task, i.e. N14: the registered +/-12% band is finer than the instrument that measures it, the seat's FOURTH prereg-band defect after D6 P2, D7 P3 and D8 P4's wording. HIT on every other limb: 3/3 TASKCMPLT, relaxation APPLIED x3, T_R5_PL1 -0.6% and T_R5_CO1 +0.05% vs D8 (indistinguishable on the sim clock), children still exactly 201.8-201.9 m from the parent at 120/240/360 deg.
P3 HIT, with 3.1 s of margin: deferrals 2.064 / 2.396 / 2.400 s, a DROP of about 2.7-2.8 s vs D8 (not the registered rise, so that finding clause does not fire). The drop is REAL and MEASURED; its cause is NOT separated by this run - the re-created platoon objects became observable at order+1.8 s here against D8's order+5.6 s, while D9's back end ran the pre-order phase at about 6.1x sim/wall against D8's ~3.5x, so a faster back end explains a large part of the drop but the STP-852 restructure is neither excluded nor attributed.
P4 HIT, cleanly: movement-window ratio 3.195 (all four per-60 s lines 3.201-3.984 also inside the D8 load profile); no split verdict this time.
P5 HIT on every limb: deployed build 4f1f149 not dirty; vendor log copies correctly under vendor/, none flat; the Stage 2h HELD line quotes the matched (garbled) rtiexec text, closing D8's N9 live; the tile census prints the run total with the outstanding-worker clause, closing D8's N8 live; jam instrument PASS (BlockedByVehicle 6 rows/1 object, Loop to stall 0, Global Replan 0); clean teardown, no BACK END LOST, no fault artefact for this run's back-end pid.
N13 (STP-855): the composed-aggregate route origin is read inside the parent's own re-compose transient, R/2 then R/4 off its authored coordinate for several seconds after its children are delete/re-created; D9 lost the timing race (50.4 m), D8 and D5c won it by margins of 0.5 s and 0.39 s respectively - not a fix, a coincidence of timing on both sides.
N14 (new): a +/-12% completion band is finer than the arrival-check's own 5 WALL s quantum on the shortest task (14-18% of its value); future bands must register no tighter than one cadence step on the shortest task, or score the crossing instant instead of the close instant.
N15 (new): the app's own C13 log line prints "2 platform(s) created in full" when there is exactly one platform in this init - it mislabels a composed aggregate whose CreateSubordinates was already false, and this pre-existing (D8, D5b) mislabelled line is what the prereg's "4 empty shells" quote was copied from.
...
### D10 RESULT (run 20260921T113151Z) - NO MISS

P1 HIT: T_R5_CO1 route 1,112 m / bar 556 / radius 278; terrain-profile reply #0 = 34.64763,-116.69339, 0.25 m off the authored coordinate (5-dp print rounding); the origin re-derived from the app's own roster is 0.042 m from authored, three orders of magnitude inside the registered ~5 m bound.
P2 HIT: exactly one ROUTE ORIGIN line (app:313), verbatim header match, three-child roster with lat/lon to 6 dp and provenance, gap stated as a number (50.5 m), zero FALLING BACK; the gap wording is the RE-COMPOSE TRANSIENT clause, EXPECTED on this run, and the provenance is the EXPECTED all-reflection-proven ending. THE FIX WAS STRESSED: independently verified from the WatchVrf trace, the parent's published position sat 50.0 m off authored at bearing 119.5 deg at the sample nearest the read (D9's defect was 50.4 m at 119.9 deg) - this run landed inside the exact transient that produced N13/STP-855, and the route still came out right. STP-855 is closed by evidence, not by timing.
P3 HIT: both controls keep 1,156 m / bar 578 / radius 289 with no ROUTE ORIGIN line; the barrier fires once before "Init dispatched" quoting both the applied 20 s cap and the configured 60 s, each labelled; READY TO TASK fires once (0.8 s) before the order; every forbidden string is zero; order-time materialization counts (4+4+4+1, 13 lines) match D9 and D8 exactly.
P4 HIT: the barrier's "(5 empty shell(s))" and the C13 line's "5 unit(s) created as EMPTY shells ... 1 platform(s) created in full" now agree - N15 is fixed, the C13 line's mislabelled second platform is gone.
P5 HIT: 3/3 TASKCMPLT by arrival evidence, relaxation APPLIED x3; T_R5_PL1 486.2 SIM s vs D9's 482.8 (+0.70%, inside +/-12%); T_R5_TK1 and T_R5_CO1 reported per N14, not scored (CO1's own bar/radius changed by design with the corrected route).
P6 reported: four delete-issued-to-readable round trips 1.684-1.784 s (mean 1.712, D9 ~1.80 s, D8 ~5.63 s) - the unexplained dispatch-deferral drop is now LOCATED but not attributed: 88% of TK1's 2.017 s deferral (1.78 s) is the back end's own re-create round trip, and D8's equivalent interval is bounded at least 4.10 s, so at least 2.3 s of the ~2.7 s drop sits in the back end, not in app-side dispatch classification; the correlated cause (D8's scenario clock ran ~2.4x slower at dispatch than D9/D10) is a correlation across three runs, not yet an attribution - a run whose pre-order rate matches D8's ~3.5x would falsify the back-end explanation if the deferral stays near 2 s. Movement-window ratio 3.219 (D9 3.195, D8 3.362, D5c 3.273) - no dead-back-end signature. Jam instrument PASS (BlockedByVehicle 8, Loop to stall 0, Global Replan 0). Give-ups 4, same four objects as D8/D9. N6 recurs at 515 rows (D7 441, D8 500, D9 494), still unattributed.
N16 (new, instrument not behaviour): the gate script hashes only VrfC2SimApp.exe, the apphost stub - the managed code is in the .dll, already recorded separately in the manifest as host.deployedAppBuild.sha256. Nothing was in doubt for D10 (both artefacts carried the same commit and mtime), but the CONDITIONS clause should name or hash the .dll before this gate is relied on to catch a C#-only code swap. Gate script fixed.
CONDITIONS differences, neither judged a violation: no rtiAssistant present (D9 ran with one left over from D5b - D10 is cleaner on that axis); the persistent holder was re-armed (RtiProbe pid 87616) AND the runner still started its own Stage 2h holder (pid 84532) - both present, the create became a join on attempt 1/4 in 3 s.
UNEXPLAINED, flagged not footnoted: T_R5_CO1's completion ran +3.2% SIM vs D8 on an identical route with identical farthest travel and member counts; N6's 515 rows, fourth run in a row with no control; the back-end round-trip difference behind the deferral drop, bounded but not attributed.
BOTTOM LINE: every HIGH limb HIT, the fix was demonstrably stressed under the same transient that produced N13, and STP-855 is closed by evidence. The prereg's own AFTER-D10 clause stands: D5d is still required to exercise the name-registry provenance branch, which this run did not reach (all three children here were reflection-proven).
