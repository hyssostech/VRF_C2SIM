...
| V2f B7 heading/speed (validation V4) | **PASS** | scratchpad validation/b7_harvest.py over the 19,712 PositionReports (all carry HeadingAngle, Speed, SubjectEntity): for 11 moving entities and ~1,200 report pairs with >= 15 m displacement, median |HeadingAngle - course derived from consecutive positions| = **0.1 deg** (degrees, correct sense; 1,300 of 1,400 pairs within 45 deg, the rest at turns). Reported Speed peaks 9.7-14.5 m/s (35-52 km/h, vehicle-plausible). Reported/derived speed ratio = 0.48 in the first 2 min and 0.78 after 14 min - it tracks the sim ratio (2.0x falling to ~1.3x) exactly as it must if Speed is the sim's TRUE ground speed in m/s and the derived value is inflated by wall-clock sampling. So Speed is m/s of sim motion, not a wall estimate. |
