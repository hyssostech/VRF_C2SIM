...
DEPLOY SET:
  ...
  * IT READS `BackendControlState` BY REFLECTION, ON PURPOSE - and since that re-pin the read-back
  ...
  `build/Release/VrfBridge.dll` and had NO BridgeConfig property, so `-p:BridgeConfig=Release-5.2`
  was SILENTLY IGNORED: they built into `bin\Release\` against the 5.0.2 bridge, reported Build
  succeeded / exit 0, and never produced a `bin\Release-5.2\` at all (measured by gate G-A,
  2026-09-14; commit `529fe5c` had converted only five tools, and VrfC2SimApp already had the
  axis). They also did not compile `tools/Shared/StackIdentity.cs`, which is what makes a tool read
  the bound stack from `VrfBridge.NativeStackInfo()` and join the 5.2 way - so the csproj axis
  ALONE would have emitted a "5.2" build that joined with the hard-coded 5.0.2 `CWIX-2024` /
  `RPR_FOM_v2.0_1516-2010.xml` / 3-module identity, which is worse than having no 5.2 build. The
  conversion therefore did BOTH: the csproj axis AND the stack-aware identity. `SmokeTest` never
  calls `Start()`, so it had no federation identity to fix - it now REPORTS the bound stack instead.
  `ResetVrf` additionally lost its baked-in default `applicationNumber 3299` (a default invites
  silent reuse of a burned appNo): a missing appNo is now exit 2, as it already was for SetSimRate.

  THE SELF-TEST (offline, joins nothing): every one of the four answers `--help` with the bound
  stack, e.g. `native stack = 5.2|C:\MAK\vrforces5.2d\bin64\vrfcontrol.dll` and
  `stack=5.2  federation=(from connection config; execName MAK-ONE-2025 expected)`. Built as
  `-c Release` the SAME sources print `5.0.2` / `federation=CWIX-2024` - that contrast is the proof
  the stack is read from the loaded DLLs and not from a build flag. `SetSimRate --dry-run` and
  `CreateTaskAgg --dry-run` print the full plan and exit without joining. `ResetVrf --dry-run`
  KEEPS its older meaning - it JOINS and discovers but issues no deletes - because that is the
...
NEW PIN 2026-09-15 16:52Z (STP-832 merge 13e8c73): VrfBridge.dll 90272BC95297E330 (1,001,472 B, mtime 16:52:06Z) - VrfFacade::Start now returns false with LastStartError on a refused create-or-join (DtExerciseConn InitializationStatus out-parameter, vl/exerciseConnHLA.h:84-109) instead of taking VR-Link's DtFatalError/DtAbort path; ELEVEN consumers at that hash (rebuilt 16:52:50-16:53:13Z, 0 errors, the 6 documented VrfC2SimApp warnings, deployed appsettings.json byte-identical to src); member presence proved on the main-built DLL by ECMA-335 metadata (VrfC2Sim.VrfBridge.LastStartError PRESENT, Start/Stop/Tick/BackendCount/NativeStackInfo still present, the negative control absent); suites 20/20 exit 0 under the 5.2 PATH prefix (rulings 176 PASS / 0 FAIL, routeshift 83 ok / 0 fail, liveness 25 PASS / 0 FAIL with the --disabled fail-first arm exit 1 / 11 FAILED); RunnerTurnaround 235 passed / 1 failed - the single FAIL is a STALE TEST ASSERTION, not a regression: the check 'StartRtiExec52 defaults to the 4001/4001/5000 rendezvous' still asserts ForwarderPort 5000 after c0c4185 (15:50Z) moved the product to 5002 (9c below), it is pre-existing on main, it is untouched by this merge, and it was left for the pin's owner to decide (the 'golden connection' value is a documented invariant, not a test-environment defect); previous pin E3F40524... (997,376 B) SUPERSEDED.
POST-PIN MANAGED REBUILD 2026-09-26 19:58Z (lane R, main 93873fd; no live run): TEN consumers REBUILT IN PLACE with -c Release -p:BridgeConfig=Release-5.2 -t:Rebuild, 0 errors (VrfC2SimApp: the documented 4x CA2024 + 2x CS8632) - VrfBridge.dll 90272bc9... (the pin; bridge source unchanged since 13e8c73) in every output tree; VrfC2SimApp ProductVersion 1.0.0+git.93873fd.Release-5.2 (not DIRTY), VrfC2SimApp.exe sha256 68e8ae1c4c1d787ef0d797a7bb93fbebf35135aa75367be8571d39b06f5c4766, VrfC2SimApp.dll sha256 a93b5a7b7d397d303ba34d4489e6b231c3a35e364f430a8bf6fd21f075432868, deployed appsettings.json byte-identical to src; `DOTNET_ENVIRONMENT=Demo --runtime-check` -> "runtime-check: OK", exit 0. THE ELEVENTH, tools/RtiProbe, was NOT rebuilt in place: its bin\Release-5.2 tree is held open by the running persistent federation holder (RtiProbe pid 34096, appNo 5103 - never killed); its source and tools/Shared are unchanged a9d738f -> 93873fd, its tree stays the a9d738f build of 2026-09-26 11:35Z at the pin, and a -t:Rebuild into a scratch output compiled 0 errors / 0 warnings at 93873fd. Rebuild it in place once the holder has resigned.
