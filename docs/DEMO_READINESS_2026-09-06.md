...
| 10 | Terrain + real-time fixture | V | MAK Earth (online) needs internet; an offline fallback terrain (a shipped SharedData terrain) should be named in the demo runbook. Real-time fixture = one builder call: `tools/FixtureGen/build_fixture.py --profile 5.2 --empty --frame-mode variable-frame` (FRAME_MODES include variable-frame; the test fixture is fixed-frame-run-to-complete, 7-10x on R9). Owed: build + deploy + one run |
